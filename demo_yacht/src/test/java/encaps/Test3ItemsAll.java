...
//		int lim = 3;
//		for (Item item : list) 
		Optional<Item> opt = repoItem.findById(1);
		if (opt.isPresent()) {
			Item item = opt.get();
//			if (--lim <= 0)
//				break;
...
//		Item item = new Item();
		...
		String s = ItemParser.parseRegex(showFax(driver), "Fax: *(.+?) ");
		item.setSelerFax(s);
		
		s = ItemParser.parseRegex(html, ">Price: *</td><td.*?>(.+?)</td>");
		item.setPrice(s);
		...
		s = ItemParser.parseRegex(html, ">Website: *</td><td.*?>.*?href=\"(.+?)\"</td>");
		item.setSelerWebsite(s);

		s = ItemParser.parseRegex(html, ">Vessel type: *</td><td.*?>(.+?)</td>");
		item.setType(s);
...
//					WebElement element = driver.findElement(By.id("id_of_element"));
					...
					Thread.sleep(1000);
					String s = ItemParser.parseRegex(driver.getPageSource(), ">Fax:<(.+?)");
